...
WOLFSPDM_API int wolfSPDM_RespInit(WOLFSPDM_RESP_CTX* ctx);
WOLFSPDM_API void wolfSPDM_RespFree(WOLFSPDM_RESP_CTX* ctx);
...
WOLFSPDM_API int wolfSPDM_RespGetCtxSize(void);

WOLFSPDM_API int wolfSPDM_RespSetMode(WOLFSPDM_RESP_CTX* ctx,
    int useTcg, int usePsk);

WOLFSPDM_API int wolfSPDM_RespSetPSK(WOLFSPDM_RESP_CTX* ctx,
...
WOLFSPDM_API int wolfSPDM_RespSetIdentityKey(WOLFSPDM_RESP_CTX* ctx,
    const byte* privKey, word32 privSz,
    const byte* pubKey, word32 pubSz);

WOLFSPDM_API int wolfSPDM_RespSetTpmCallback(WOLFSPDM_RESP_CTX* ctx,
    WOLFSPDM_RESP_TPM_CB cb, void* userCtx);

WOLFSPDM_API void wolfSPDM_RespSetDebug(WOLFSPDM_RESP_CTX* ctx, int enable);
...
WOLFSPDM_API int wolfSPDM_RespHandleMessage(WOLFSPDM_RESP_CTX* ctx,
    const byte* inBuf, word32 inSz,
    byte* outBuf, word32* outSz);

WOLFSPDM_API void wolfSPDM_RespReset(WOLFSPDM_RESP_CTX* ctx);

/* SPDMONLY lock: when 1, the transport must reject plaintext TPM frames.
 * Toggled by the requester via SPDMONLY vendor command. */
WOLFSPDM_API int wolfSPDM_RespIsLocked(const WOLFSPDM_RESP_CTX* ctx);

/* Returns 1 when a secured SPDM session is established. */
WOLFSPDM_API int wolfSPDM_RespIsSessionActive(const WOLFSPDM_RESP_CTX* ctx);
...
WOLFSPDM_API word32 wolfSPDM_RespGetIdentityKey(const WOLFSPDM_RESP_CTX* ctx,
    const byte** idPub);
